...
#brain = Brain(5,3,0.9) # 5 sensors, 3 actions, gama = 0.9
...
def init():
    ...
    brain = Brain(80, 60, 3, 0.9)

    sand = np.zeros((longueur,largeur), dtype=np.float32) # initializing the sand array with only zeros
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        action = brain.update(sand, int(self.car.x), int(self.car.y), goal_x, goal_y, last_reward)
